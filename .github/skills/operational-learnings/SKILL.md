...
跨切面通用教训（不绑定特定 feature）的主存储路径为 `.github/experience/`；feature 专属教训仍存放于 `docs/specs/<feature>/artifacts/operational-learnings/`。
...
| Project Local (Feature) | feature 专属教训 | `docs/specs/<feature>/artifacts/operational-learnings/` | 重复出现或高影响 |
| Project Local (Cross-Cutting) | 跨切面通用教训（平台坑、工具链迁移、MCP 配置等） | `.github/experience/` | 重复出现或高影响 |
...
| OL-R6 | 教训跨多个 feature/模块 | 存入 `.github/experience/`；文件名用 kebab-case 描述主题 | 塞入某 feature 专属路径 |
