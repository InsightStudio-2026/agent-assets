...
> 智能体自定义体系中央注册表。VS Code Copilot 与 Claude Code 等多 Agent 环境共用入口。
> 权威 SSOT 始终为 `.github/instructions/` 目录下的指令文件。
...
> 自动注入对话上下文的项目级规范。三种加载机制：`applyTo` 文件模式匹配、`description` 语义任务匹配、手动附加。
...
> 语义自动命中或手动触发的知识包。置于 `.github/skills/` 下自动发现。
...
> 项目运行教训与跨平台踩坑记录，由 `operational-learnings` skill 统一管理生命周期（记录 → 修剪 → 晋升）。

| 经验 | 说明 | 路径 |
| ------ | ------ | ------ |
| claude-code-mcp-migration | Claude Code MCP 迁移配置教训 | [claude-code-mcp-migration.md](.github/experience/claude-code-mcp-migration.md) |
